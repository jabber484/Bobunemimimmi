...
void *sender_ackListener(void *data){
	...
	while(1){
		...
		pthread_create(&timeThread, NULL, waitForTimeOut, mygbn_sender);
		...
		if(response->type == AckPacket){
			...
			if(headSeqNum <= response->seqNum && headSeqNum > 0){/* Slide Right */
       			//++++++++++++++++++++NEW+++++++++++++++++++
				printf(" Because of %d <= %d\n",headSeqNum, response->seqNum);
